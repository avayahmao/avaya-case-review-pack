...
The v1.5.0 report contract remains the presentation baseline: a clean executive body, no generated Risk Flags or Targeted Recommendations, and a final reverse-mapped Evidence Appendix.
...
6. Keep standard non-ADM output unchanged.
...
All four dimensions must be covered somewhere in the final output when evidence permits. Unsupported content remains `不知道`, `Unknown`, or explicitly pending validation.
...
- **Verdict:** concise outcome only; do not overload with ADM detail.
...
- **Timeline:** preserve substantive diagnostic chronology.
- **Appendix A — Evidence Register:** remain the final section and reverse-map all ADM-derived factual claims.

No top-level headings named `Details/Findings`, `Problem Clarification`, `Cause`, or `Solution` are required. The agent may use a contextual subheading when it improves readability, but it must not output a mechanical four-part form.
...
ADM depth should appear through richer content inside those natural email sections. Do not append a second four-section ADM report after the email.
...
- If no verifiable case-specific evidence exists, output exactly `不知道`.
...
- **No ADM request:** use the standard v1.5.0 output with no extra depth requirement.
...
9. Zero evidence still produces exactly `不知道`.
