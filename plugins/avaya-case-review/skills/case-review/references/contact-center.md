...
|---------|------|----------------|--------------|
| **AACC (Aura Contact Center)** | Contact center routing, agent management | CVLAN to CM, SIP, ASAI | AACC logs (`C:\Avaya\Logs\` or `D:\Avaya\Contact Center\<Component>\Logs\`), SCMU, CM SAT vectors |
...
Step 2 — Verify Agent State in AACC
  - CCMA Supervisor Desktop: agent should show "Ready" state
  ...
  - If supervisor logout fails: use SCMU (System Control and Monitor Utility)
    to check CCMS / CCT service health; restart stuck service if needed
...
Step 3 — Check for Common Issues
  ...
  - Ghost sessions: status station <ext> shows busy when agent is idle
    Fix: manual release of stuck port/session via CCMA Supervisor force-logout
  - Browser cache: stale cache can show incorrect agent state in UI
  - AACC is Windows Server — service management via SCMU GUI or
    Windows Services (services.msc), NOT Linux ccsm/service commands
...
## AACC Stuck Agent Release

**Workflow 27: AACC Stuck Agent — Official Release Procedures (Windows)**

```
"Stuck agent" = agent session persists in AACC in a state (Busy/ACW/Not Ready)
that cannot be changed by the agent; may also have CM station stuck busy.
Customer workaround of full server reset is never required for this class of issue.

Tier 1 — CCMA Supervisor Desktop Force Logout (no CLI, no admin access)
  1. Log into CCMA → Supervisor Desktop → Real-Time Displays
  2. Locate stuck agent in agent state panel
  3. Right-click agent → "Log Out Agent"
  4. AACC sends logout command; session + license released immediately
  5. Verify: agent disappears from Active sessions, license count decreases by 1
  PRIMARY OFFICIAL PROCEDURE — try this first, every time.

Tier 2 — CM SAT Station Cleanup (if station also shows busy on CM side)
  status station <ext>            ! Confirm: station busy with no visible call
  busyout station <ext>           ! Force station out of service
  release station <ext>           ! Release any phantom port allocation
  status station <ext>            ! Confirm: back to idle
  Do BOTH Tier 1 + Tier 2 if CCMA shows stuck AND CM station shows busy.

Tier 3 — AACC Windows Service Restart (targeted, no server reboot)
  AACC is Windows Server — use SCMU or Windows Services, NOT Linux commands.

  Via SCMU GUI (preferred):
    Start → All Programs → Avaya → Contact Center → System Control and Monitor Utility
    Select CCMS tab → Stop service → wait for stopped → Start service
    Or CCT tab for agent desktop session issues

  Via Windows Services CLI (as Administrator):
    net stop "Avaya Contact Center Manager Server"
    net start "Avaya Contact Center Manager Server"
    (verify exact service names on this server: Get-Service -DisplayName "Avaya*")

  Restart CCMS for: routing engine / agent state database stuck records
  Restart CCT for:  agent desktop session / JTAPI connection issues
  Restart CCMM for: multimedia contact session issues

Tier 4 — HA Switchover (if Tier 1-3 fail and site has HA Resiliency)
  A controlled HA switchover clears all active session state on the active node.
  Less disruptive than full server reset; standby takes over call handling.
  Via SMMC system tray on HA server → "Enable Switchover"
  Note: verify standby is healthy BEFORE switchover (SCMU on standby node).

Trigger Pattern Diagnosis:
  Transfer/conference + call drop    → Tier 1 + Tier 2 (phantom call remnant)
  Agent desktop crash without logout → Tier 1 (orphaned AACC session)
  CCMS service hiccup                → Tier 3 (CCMS restart clears DB record)
  Recurring at scale                 → collect CCMS logs + Windows Event Viewer
                                       for root cause; may indicate CVLAN link
                                       flap or CCMS database contention
```
...
**AACC (Avaya Aura Contact Center) runs on Windows Server — all paths are Windows.**
Drive letter varies by installation (`C:` or `D:`). Default install base: `D:\Avaya\Contact Center\`.

```
# Consolidated logs (all components)
C:\Avaya\Logs\

# Per-component log directories (under install base)
D:\Avaya\Contact Center\Manager Server\Logs\         # CCMS — core routing engine
D:\Avaya\Contact Center\Multimedia Server\Logs\      # CCMM — multimedia/email contacts
D:\Avaya\Contact Center\Communication Control Toolkit\Logs\  # CCT — agent desktop integration
D:\Avaya\Contact Center\Manager Administration\Logs\ # CCMA — web UI / IIS

# Installer / patch logs
C:\Avaya\Logs\Sysops\MsiLogs\

# Windows system logs (check after service failures or outages)
Windows Event Viewer → Application and System logs
```

**SCMU (System Control and Monitor Utility)** — primary AACC Windows management tool:
```
Start → All Programs → Avaya → Contact Center → System Control and Monitor Utility
Tabs: LM | CCMS | CCT | CCMM | CCMA
All services must show green/running for normal operation.
```

**SMMC (System Management and Monitoring Component)** — HA management (system tray):
```
Used on HA servers to: Stop HA System / Start HA System / Enable Switchover
```

**SGM Client** — verify AES + Session Manager connectivity:
```
D:\Avaya\Contact Center\Manager Server\iccm\sgm\management client\SGMManagementClient.jar
Run: java -jar SGMManagementClient.jar
Expected: "connected" for both AES and Proxy (Session Manager)
```
